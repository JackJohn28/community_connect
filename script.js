        ...
        role === "volunteer"
          ? `<input type="text" id="reg-expertise" placeholder="Expertise (e.g. Nursing, Tech)">
           ...
           <div style="display: flex; gap: 10px; margin-top: 4px;">
             <div style="flex:1;"><label style="font-size:0.75em;">Available from</label><input type="time" id="reg-avail-start"></div>
             <div style="flex:1;"><label style="font-size:0.75em;">Available until</label><input type="time" id="reg-avail-end"></div>
           </div>`
  ...
  if (target) {
    ...
    if (id === "profile") {
      if (typeof renderProfile === "function") {
        renderProfile();
        if (currentUser.role === "volunteer") renderVolunteerHours();
      }
    }
  }
}

// --- 6. NOTIFICATION SYSTEM (UCN 7) ---

// Start real-time listener for incoming notifications
...
function showToastNotification(notif) {
  ...
  toast.innerHTML = `
    <div style="display:flex; align-items:flex-start; gap:10px;">
      ...
      <div>
        <p style="font-weight:600; margin:0 0 2px; font-size:0.9rem;">${notif.title}</p>
        <p style="margin:0; font-size:0.82rem; opacity:0.85;">${notif.message}</p>
      </div>
      <button onclick="dismissToast()" style="background:none;border:none;cursor:pointer;color:white;font-size:1.1rem;margin-left:auto;padding:0 0 0 8px;">✕</button>
    </div>`;
...
// Send a notification to a user (called by org)
async function sendNotification(toUid, title, message, type = "info") {
  try {
...
async function renderNotifications() {
  ...
  if (!container) return;
  container.innerHTML = `<p style="color:#666; font-size:0.9em;">Loading notifications...</p>`;
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      html += `
        <div class="notif-item ${n.read ? "" : "notif-unread"}" onclick="markRead('${doc.id}', this)">
    ...
    container.innerHTML = `<p style="color:#c53030;">Error loading notifications.</p>`;
  }
}
...
// --- 7. UCN 8: POSITION STATUS MANAGEMENT ---

// Accept or deny a volunteer from org management panel
async function updateVolunteerApplication(
  docId,
  roleIdx,
  volunteerUid,
  volunteerName,
  ...
  try {
    ...
    if (action === "accepted") {
      // Already in volunteers array — just notify
    } else if (action === "denied") {
      // Remove from volunteers array
      positions[roleIdx].volunteers = pos.volunteers.filter(
        (uid) => uid !== volunteerUid,
      );
      await docRef.update({ positions });
    }

    // Send push notification to volunteer (UCN 7)
    const title =
      action === "accepted" ? `Application Accepted` : `Application Update`;
    const message =
      action === "accepted"
        ? `Your application for "${pos.roleName}" at "${data.title}" has been accepted!`
        : `Your application for "${pos.roleName}" at "${data.title}" was not selected at this time.`;

    await sendNotification(volunteerUid, title, message, action);

    alert(`Volunteer ${action}. Notification sent to ${volunteerName}.`);
    renderOrgManagement();
  } catch (e) {
...
// Update position status (open / filled / closed) — UCN 8
async function updatePositionStatus(docId, roleIdx, newStatus) {
  const docRef = db.collection("resources").doc(docId);

  try {
    const snap = await docRef.get();
    const positions = snap.data().positions;
    positions[roleIdx].status = newStatus;
    await docRef.update({ positions });

    // Notify all volunteers in that role about the status change
    const pos = positions[roleIdx];
    const listingTitle = snap.data().title;
    for (const vUid of pos.volunteers || []) {
      await sendNotification(
        vUid,
        `Position Status Updated`,
        `The "${pos.roleName}" position for "${listingTitle}" is now marked as ${newStatus.toUpperCase()}.`,
        "status_update",
      );
    }

    alert(`Position status updated to "${newStatus}". Volunteers notified.`);
    renderOrgManagement();
  } catch (e) {
    console.error("Error updating position status:", e);
    alert("Failed to update position status: " + e.message);
  }
}
  ...
  const tips = {
    ...
    org: [
      ...
      },
      {
        ...
        desc: "View all your posts, update position statuses, and accept or deny volunteers.",
      },
      {
        icon: "👥",
        ...
        res.positions.forEach((pos, idx) => {
          ...
          const matchBadge =
            currentUser.role === "volunteer" && skillRequired
              ? `<span style="display:inline-block; margin-left:6px; font-size:0.65em; font-weight:700; padding:2px 7px; border-radius:4px; text-transform:uppercase; letter-spacing:0.05em; background:${isMatch ? "#d1fae5" : "#f1f4f9"}; color:${isMatch ? "#065f46" : "#4a6580"};">${isMatch ? "Matches your skills" : `Needs: ${skillRequired}`}</span>`
              : skillRequired
          ...
          const statusColors = {
            open: "#d1fae5|#065f46",
            filled: "#fef3c7|#92400e",
            closed: "#fee2e2|#991b1b",
          };
          const [statusBg, statusColor] = (
            statusColors[posStatus] || statusColors.open
          ).split("|");
          const statusBadge = `<span style="font-size:0.65em; font-weight:700; padding:2px 8px; border-radius:4px; text-transform:uppercase; letter-spacing:0.05em; background:${statusBg}; color:${statusColor}; margin-left:6px;">${posStatus}</span>`;
          ...
          rolesHTML += `
            <div style="background: #f1f4f9; padding: 10px; border-radius: 8px; margin-top: 10px; border-left: 4px solid #6e84a3;">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                  <h4 style="margin:0;">${pos.roleName} <span class="urgency-tag urgency-${pos.urgency}">${pos.urgency}</span>${statusBadge}${matchBadge}</h4>
                ...
                ${
                  currentUser.role === "volunteer"
                    ? `<button class="primary-btn" style="width:auto; padding:5px 10px;"
                      onclick="signUpForRole('${docId}', ${idx})" ${!canJoin ? "disabled" : ""}>
                      ${userSigned ? "Joined" : posStatus !== "open" ? posStatus : isFull ? "Full" : "Join"}
                    </button>`
                    : ""
                }
              </div>
...
async function renderOrgManagement() {
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      for (let roleIdx = 0; roleIdx < positions.length; roleIdx++) {
        const pos = positions[roleIdx];
        const posStatus = pos.status || "open";
            ...
            try {
              ...
              if (vDoc.exists) {
                ...
                const vMatchBadge = reqSkill
                  ? `<span style="float:right; font-size:0.7em; font-weight:700; padding:2px 7px; border-radius:4px; text-transform:uppercase; letter-spacing:0.04em; background:${vMatch ? "#d1fae5" : "#fff7ed"}; color:${vMatch ? "#065f46" : "#92400e"};">${vMatch ? "Skill match" : "Skill gap"}</span>`
                  : "";
                  ...
                  <div style="font-size: 0.9em; background: white; padding: 8px; border-radius: 4px; margin-top: 5px; border: 1px solid #eee;">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px; flex-wrap:wrap;">
                      ...
                      <div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:4px;">
                        <button class="primary-btn" style="width:auto; padding:4px 10px; font-size:0.78em; white-space:nowrap; margin-top:0; background:linear-gradient(135deg,#22c55e,#16a34a);"
                          onclick="updateVolunteerApplication('${doc.id}', ${roleIdx}, '${vUid}', '${vFullName.replace(/'/g, "\\'")}', 'accepted')">
                          ✓ Accept
                        </button>
                        <button class="secondary-btn" style="width:auto; padding:4px 10px; font-size:0.78em; white-space:nowrap; margin-top:0; color:#c53030; border-color:#fecaca;"
                          onclick="updateVolunteerApplication('${doc.id}', ${roleIdx}, '${vUid}', '${vFullName.replace(/'/g, "\\'")}', 'denied')">
                          ✕ Deny
                        </button>
                        <button class="primary-btn" style="width:auto; padding:4px 10px; font-size:0.78em; white-space:nowrap; margin-top:0;"
        ...
        // UCN 8: Position status controls
        const statusOptions = ["open", "filled", "closed"];
        const statusControlHTML = `
          <div style="display:flex; align-items:center; gap:8px; margin-top:10px; padding-top:8px; border-top:1px solid #e2e8f0;">
            <label style="font-size:0.75em; font-weight:600; text-transform:uppercase; color:#6e84a3; margin:0;">Position Status:</label>
            <select onchange="updatePositionStatus('${doc.id}', ${roleIdx}, this.value)"
              style="width:auto; padding:4px 28px 4px 10px; font-size:0.82em; margin:0;">
              ${statusOptions.map((s) => `<option value="${s}" ${posStatus === s ? "selected" : ""}>${s.charAt(0).toUpperCase() + s.slice(1)}</option>`).join("")}
            </select>
          </div>`;
